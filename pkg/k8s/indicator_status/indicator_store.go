...
func NewIndicatorStore() *IndicatorStore {
	return &IndicatorStore{indicators: make([]v1alpha1.Indicator, 0)}
}

func (is *IndicatorStore) Add(indicator v1alpha1.Indicator) {
	is.Lock()
	is.indicators = append(is.indicators, indicator)
	is.Unlock()
}

func (is *IndicatorStore) Delete(indicator v1alpha1.Indicator) {
	nextIndicators := make([]v1alpha1.Indicator, 0)
	is.Lock()
	defer is.Unlock()
...
func (is *IndicatorStore) Update(indicator v1alpha1.Indicator) {
	is.Lock()
	defer is.Unlock()
...
func (is *IndicatorStore) GetIndicators() []v1alpha1.Indicator {
	is.Lock()
	indicators := make([]v1alpha1.Indicator, 0, len(is.indicators))
